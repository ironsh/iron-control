...
class Iron::BootstrapTest < ActiveSupport::TestCase
  ...
  setup do
    ...
    @env = ENV.to_hash.slice(
      "IRON_BOOT_INITIAL_USER_EMAIL",
      "IRON_BOOT_INITIAL_USER_PASSWORD",
      "IRON_BOOT_INITIAL_API_KEY"
    )
  end

  teardown do
    %w[IRON_BOOT_INITIAL_USER_EMAIL IRON_BOOT_INITIAL_USER_PASSWORD IRON_BOOT_INITIAL_API_KEY].each do |k|
      ENV.delete(k)
    end
  ...
  def set_env(email: nil, password: nil, api_key: nil)
    ENV["IRON_BOOT_INITIAL_USER_EMAIL"] = email
    ENV["IRON_BOOT_INITIAL_USER_PASSWORD"] = password
    ENV["IRON_BOOT_INITIAL_API_KEY"] = api_key
  end
